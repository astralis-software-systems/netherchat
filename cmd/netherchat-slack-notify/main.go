// Command netherchat-slack-notify posts Slack notices when a Netherchat war room
// opens, seals, or scuttles (NC-5, outbound notify). It is the Slack-native twin of
// netherchat-teams-notify: it joins the room as an ordinary, DECRYPTING member and
// reacts to typed events — but instead of a Teams Adaptive Card it renders a Slack
// Block Kit message and POSTs it to an incoming webhook.
//
// THE BOUNDARY LAW: a message carries only a pointer (a one-time join link) and
...
// Although this process can decrypt the room (it is a member), the message mappers
// read only event metadata — the seal message uses the chain head hash and signer
...
const defaultConfigFile = "netherchat-slack.toml"

// validNotifyEvents is the closed set of events the Slack notifier understands.
...
// notifier maps decrypted room events to Slack messages. requestInvite and
// clockElapsed are injected (the live client's methods in main, stubs in tests).
type notifier struct {
	room    string
	on      map[string]bool
	webBase string
	expires string // room TTL captured at connect, shown on the open message
...
// messages returns the Slack messages to POST for one client event (usually zero or
// one). It is the whole event→message policy, kept free of I/O so it is unit-testable.
func (n *notifier) messages(ev client.Event) [][]byte {
	switch e := ev.(type) {
	...
	case client.EvServerMessage:
		...
		// (NC-1). That is our "open" trigger; we mint a one-time link to put on the message.
	...
	case client.EvInvite:
		...
		p := n.pendingOpen
		n.pendingOpen = nil
		msg := slack.OpenMessage(slack.OpenMeta{
			Room: e.Room, Severity: p.severity, Source: p.source,
	...
	case client.EvScuttleReceipt:
		...
		return [][]byte{n.scuttleMessage(e)}
	}
	return nil
}

// sealMessage builds the seal message from ONLY the record's metadata — the head
// hash and the signer count. It never touches the sealed entries (the decisions).
func (n *notifier) sealMessage(e client.EvSealComplete) []byte {
	...
	return slack.SealMessage(slack.SealMeta{Room: n.room, Signers: signers, RecordHash: hash, Elapsed: elapsed})
}

// scuttleMessage builds the scuttle message from the signed receipt's metadata.
func (n *notifier) scuttleMessage(e client.EvScuttleReceipt) []byte {
	...
	return slack.ScuttleMessage(slack.ScuttleMeta{Room: n.room, Actor: actor, Reason: reason, ReceiptHash: hash})
}

func main() {
	fs := flag.NewFlagSet("netherchat-slack-notify", flag.ExitOnError)
	room := fs.String("room", "", "room to watch and notify Slack about")
	webhook := fs.String("webhook", "", "Slack incoming webhook URL")
	on := fs.String("on", "", "comma-separated events: open,seal,scuttle")
	server := fs.String("server", "", "relay URL (default ws://localhost:3000)")
	name := fs.String("name", "", "display name in the room (default slack-bridge)")
	...
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "netherchat-slack-notify — post Slack Block Kit messages on room open/seal/scuttle (NC-5)")
	...
	dispName := connector.FirstNonEmpty(*name, cfg.Name, "slack-bridge")
	...
	fmt.Fprintf(os.Stderr, "netherchat-slack-notify watching #%s as %s (%s) — on %s → Slack\n",
		room0, dispName, c.Fingerprint(), strings.Join(sortedEvents(onSet), ","))
	fmt.Fprintln(os.Stderr, "messages are fire-and-forget with no on-disk queue; if this process dies, pending messages are lost (by design).")
	...
	for {
		...
		case ev := <-c.Events():
			switch e := ev.(type) {
			case client.EvError:
				fmt.Fprintf(os.Stderr, "netherchat-slack-notify: client error: %v\n", e.Err)
			case client.EvKeyReady:
				fmt.Fprintf(os.Stderr, "netherchat-slack-notify: room key established (epoch %d)\n", e.Epoch)
			}
			for _, msg := range n.messages(ev) {
				wg.Add(1)
				go func(msg []byte) {
					defer wg.Done()
					if err := slack.Post(ctx, httpClient, webhookURL, msg); err != nil {
						fmt.Fprintf(os.Stderr, "netherchat-slack-notify: post failed: %v\n", err)
					}
				}(msg)
			}
		case <-c.Done():
			fmt.Fprintln(os.Stderr, "netherchat-slack-notify: disconnected")
			wg.Wait()
			return
		case <-ctx.Done():
			fmt.Fprintln(os.Stderr, "netherchat-slack-notify: shutting down")
...
func fatal(err error) {
	fmt.Fprintln(os.Stderr, "netherchat-slack-notify: "+err.Error())
	os.Exit(1)
}
