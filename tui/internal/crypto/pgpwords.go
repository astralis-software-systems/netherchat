...
// The PGP Word List (PGPfone Biometric Word List): 256 "even" two-syllable words
// and 256 "odd" three-syllable words. A byte at an even position in a sequence is
// rendered from pgpEven; a byte at an odd position from pgpOdd. The alternation
// makes a spoken/read string resistant to transposition and confusion. It is the
// public, well-known list used by PGP/PGPfone; embedding it (no network fetch) is
// what lets two parties read a Short Authentication String to each other over a
// side channel (see sas.go).

// pgpEven holds the 256 two-syllable words (used at even byte positions).
var pgpEven = [256]string{
...
// pgpOdd holds the 256 three-syllable words (used at odd byte positions).
var pgpOdd = [256]string{
